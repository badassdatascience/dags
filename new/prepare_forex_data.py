...
# same here:
#
def save_config(config, function_name):
    filename = config['pipeline_output_directory'] + '/' + config['uid'] + '----CONFIG_' + function_name + '.json'
...
def PrepareForexData():
    ...
    def extract_candlestick_data_from_database(
        ...
        output_filename = config_dict['pipeline_output_directory'] + '/' + config_dict['uid'] + '----PANDAS_DF_' + function_name + '.parquet'
        config_dict['FILENAME_' + function_name] = output_filename
        save_config(config_dict, function_name)
