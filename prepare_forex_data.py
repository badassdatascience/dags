...
# Task:  pull candlestick data from database
import pandas as pd
from sqlalchemy import create_engine
from utilities.get_database_connection_string import db_connection_str
from utilities.get_sql_for_pull import get_candlestick_pull_query
...
debug_mode = False
limit_5 = False
run_id = 'test----309457bc-a227-4332-8c0b-2cf5dd38749c'
...
# temp
pipeline_output_directory = '/home/emily/Desktop/projects/test/badass-data-science/badassdatascience/forecasting/deep_learning/pipeline_components/output/TEMP'
...
# same here:
#
def save_config(config, suffix):
    filename = config['pipeline_output_directory'] + '/' + config['uid'] + '----CONFIG_' + suffix + '.json'
    with open(filename, 'w') as fff:
        json.dump(config, fff, indent=True)
...
@dag(
    ...
    catchup = False,
    tags=['forex', 'mysql', 'time series', 'sql'],
)
def PrepareForexData():
    ...
    def initialize(
        ...
        save_config(config_initial, inspect.currentframe().f_code.co_name)
        return config_initial


    # #####################################################
    # #   Task:  Extract candlestick data from database   #
    # #####################################################
    
    @task()
    def extract_candlestick_data_from_database(
            config_dict,
            suffix = 'candlesticks_pandas_df',
            price_type_name = 'mid',
        ...
        config_dict['mid'] = price_type_name
        config_dict['instrument_name'] = instrument_name
        config_dict['interval_name'] = interval_name

        sql_query_to_run = get_candlestick_pull_query()
        ...
        output_filename = config_dict['pipeline_output_directory'] + '/' + config_dict['uid'] + '----' + suffix + '.parquet'
        config_dict['filename_candlesticks_pull'] = output_filename
        save_config(config_dict, inspect.currentframe().f_code.co_name)
    ...
    dict_initialized = initialize(pipeline_output_directory)
    candlestick_data_dict = extract_candlestick_data_from_database(dict_initialized)
    ...
    #     import pprint as pp
    #     print()
    #     pp.pprint(candlestick_data_dict)
    #     print()
    #     print(candlestick_data_dict['pdf'])
    #     print()
